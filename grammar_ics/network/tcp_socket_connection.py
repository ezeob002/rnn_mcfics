...
from grammar_ics.network.base_socket_connection import BaseSocketConnection
from grammar_ics.utils.constants import *
        ...
        if self._server:
            ...
            try:
                ...
                if e.errno == errno.EADDRINUSE:
                    raise exception.GICSOutOfAvailableSockets()
                else:
                    raise
            ...
            try:
                ...
                if e.errno in [errno.EAGAIN]:
                    raise exception.GICSTargetConnectionFailedError(str(e))
                else:
                    raise
        else:
            try:
    ...
    def recv(self, max_bytes: int = DEFAULT_MAX_RECV) -> Any:
        ...
        try:
            ...
            if e.errno == errno.ECONNABORTED:
                raise_(
                    exception.GICSTargetConnectionAborted(socket_errno=e.errno, socket_errmsg=e.strerror),
                    None,
                    sys.exc_info()[2],
                )
            elif (e.errno == errno.ECONNRESET) or (e.errno == errno.ENETRESET) or (e.errno == errno.ETIMEDOUT):
                raise_(exception.GICSTargetConnectionReset(), None, sys.exc_info()[2])
